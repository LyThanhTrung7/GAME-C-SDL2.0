...
void game_map::load_map(const char* name) {
	FILE* fb = NULL;
	fopen_s(&fb, name, "rb");
	if (fb == NULL) return;
	...
	for (int i = 0; i < MAX_MAP_Y; i++) {
		for (int j = 0; j < MAX_MAP_X; j++) {
			fscanf_s(fb, "%d", &game_map_.tile[i][j]);
...
void game_map::load_tile(SDL_Renderer *screen) {

	char file_img[30];
	FILE* fb = NULL;

	for (int i = 0; i < MAX_TILES; i++) {
		sprintf_s(file_img, "image/map/%d.png", i);
...
void game_map::draw_map(SDL_Renderer* screen) {
	int x1 = 0;
	int y1 = 0;
	...
	for (int i = y1; i < y2; i += TILE_SIZE) {
		...
		for (int j = x1; j < x2; j += TILE_SIZE) {
			...
			if (val > 0) {
				...
				tile_mat[val].Render(screen, NULL, &renderQuad);
			}
			map_x++;
		}
		map_y++;
